...
sysname(int num)
{
  ...
  case SYS_shutdown_socket:
    return "SYS_shutdown_socket";
  case SYS_sendmsg:
    return "SYS_sendmsg";
  case SYS_recvmsg:
    return "SYS_recvmsg";
  ...
  case SYS_clockadjtime:
    return "SYS_clockadjtime";
  case SYS_setns:
    return "SYS_setns";
  case SYS_renameat2:
...
void syscall_handler(void)
{
  ...
  if (num > 0 && num < NELEM(syscalls) && syscalls[num])
  {
    ...
    if (trace)
    {
      // 不追踪 sys_write 的 STDOUT 避免日志刷屏
      if (num == SYS_write && args[0] == 1)
      {

...
int tf_attach_image(const char *path) {
    ...
    //     g_backend = NULL;
    // }
    // g_backend = fopen(path, "r+b");   /* 读写打开现有 FAT32 镜像 */
    // if (!g_backend) {
    //     fprintf(stderr, "[tf_attach_image] fopen('%s') failed\n", path);
    //     return -1;
    // }
    printf("[DBG] tf_attach_image g_backend=\n");
    return 0;
}

/* 新增：分离镜像文件 */
int tf_detach_image(void) {
    printf("[DBG] tf_detach_image g_backend=\n");
    // if (g_backend) {
    //     fclose(g_backend);
    //     g_backend = NULL;
    // }
...
/* 新增：获取后端指针 */
// FILE *tf_get_backend(void) {
//     return g_backend;
// }

// USERLAND
int read_sector(uint8_t *data, uint32_t sector) {
    // printf("[DBG] read_sector, g_backend=%p\n", g_backend);
    // fflush(stdout);
    // FILE *fp;
    // fp = g_backend;
    // if (!fp) return -1;  /* 未附着镜像 */
    // fp = fopen("test.fat32", "r+b");
    // fseek(fp, sector*512, 0);
    // fread(data, 1, 512, fp);
    // fclose(fp);
    // struct buf b;
    // b.blockno = sector;          // 目标 LBA
    // b.disk = 0;
    // /* b.data 必须是内核能够DMA的地址（通常可以直接用栈上的数组） */
    // virtio_disk_rw(0, &b, 0);    // 0 表示读
    struct buf* b = bread(0, sector);
    memcpy(data, b->data, 512);
    brelse(b);
    // memcpy(data, b.data, 512);   // 把结果复制到输出 data
    return 0;
}
...
int write_sector(uint8_t *data, uint32_t blocknum) {
    ...
    // fclose(fp);
    // struct buf b;
    // b.blockno = blocknum;    // 目标 LBA
    // b.disk = 0;
    // memcpy(b.data, data, 512);   // 要写的数据先拷进 buf
    // virtio_disk_rw(0, &b, 1);    // 1 表示写
    struct buf* b = bread(0, blocknum);
    memcpy(b->data, data, 512);
    bwrite(b);      // 不要直接调用virtio_disk_rw
    brelse(b);
    return 0;
}
...
int tf_init(void) {
    BPB_struct *bpb;
    uint32_t fat_size, root_dir_sectors, data_sectors, cluster_count;
    printf("init_in");
    /* 后端是否附着 */
    // if (tf_get_backend() == NULL) {
    //     dbg_printf("tf_init FAILED: backend not attached\n");
    //     return TF_ERR_BAD_FS_TYPE;
    // }
    
    /* 读引导扇区 */
    tf_info.currentSector = 0xFFFFFFFF;   /* 标记缓存无效 */
    tf_info.sectorFlags = 0;
    if (tf_fetch(0) != 0) {
        printf("tf_init FAILED: cannot read sector 0\n");
        return TF_ERR_BAD_FS_TYPE;
    }
    ...
    if (!((bpb->BS_JumpBoot[0] == 0xEB && bpb->BS_JumpBoot[2] == 0x90) ||
        ...
        printf("tf_init FAILED: bad jump instruction\n");
        return TF_ERR_BAD_FS_TYPE;
    }

    /* 扇区大小仅支持 512 */
    if (bpb->BytesPerSector != 512) {
        dbg_printf("tf_init FAILED: BytesPerSector=%u (only 512 supported)\n", bpb->BytesPerSector);
        return TF_ERR_BAD_FS_TYPE;
    }
    ...
    if ((bpb->Media != 0xF0) && (bpb->Media < 0xF8 || bpb->Media > 0xFF)) {
        dbg_printf("tf_init FAILED: invalid Media 0x%02x\n", bpb->Media);
        return TF_ERR_BAD_FS_TYPE;
    }

    /* FAT 大小：FAT16M 用 FATSize16，否则 FAT32 用 FAT32 的 FATSize */
    fat_size = (bpb->FATSize16 != 0) ? bpb->FATSize16
                                     : bpb->FSTypeSpecificData.fat32.FATSize;

    /* 根目录扇区数（FAT32 RootEntryCount 通常为 0） */
    root_dir_sectors = ((bpb->RootEntryCount * 32) + (bpb->BytesPerSector - 1)) / bpb->BytesPerSector;
    ...
    if (tf_info.sectorsPerCluster == 0) {
        dbg_printf("tf_init FAILED: sectorsPerCluster == 0\n");
        return TF_ERR_BAD_FS_TYPE;
    }
    ...
    if (cluster_count < 65525) {
        dbg_printf("tf_init FAILED: cluster_count=%u < 65525 (FAT32 threshold)\n", cluster_count);
        return TF_ERR_BAD_FS_TYPE;
    }

    tf_info.type = TF_TYPE_FAT32;
    tf_info.rootDirectorySize = 0;  /* 暂不计算 */
    printf("tf_init iner\n");
#ifdef TF_DEBUG
    tf_stats.sector_reads = 0;
    tf_stats.sector_writes = 0;
    dbg_printf("[DEBUG] fat_size=%u total=%u data=%u clusters=%u firstData=%u\n",
               fat_size, tf_info.totalSectors, data_sectors, cluster_count, tf_info.firstDataSector);
#endif

    /* 引导扇区签名 (末尾 55 AA) */
    if (!(tf_info.buffer[510] == 0x55 && tf_info.buffer[511] == 0xAA)) {
        dbg_printf("tf_init FAILED: missing 0x55AA signature\n");
        return TF_ERR_BAD_FS_TYPE;
    }

    dbg_printf("tf_init() successful.\n");
    return 0;
}
// int tf_init() {
//     BPB_struct *bpb;
...
int tf_mkdir(uint8_t *filename, int mkParents) {
    ...
    uint32_t psc = fp->startCluster; // store this for later
...
int tf_fclose(TFFile *fp) {
    ...
    if (!fp
        || fp == (TFFile*)-1
        || (fp < &tf_file_handles[0] || fp > &tf_file_handles[TF_FILE_HANDLES-1]))
        return -1;

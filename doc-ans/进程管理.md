...
| 文件路径 | 行数 | 主要功能 |
|---------|------|---------|
| src/proc/proc.c:318-375 | 58 | 优先级调度器主循环 |
| src/proc/proc.c:408-478 | 71 | fork系统调用 |
| src/proc/proc.c:649-671 | 23 | sched上下文切换 |
| src/proc/proc.c:676-703 | 28 | sleep睡眠机制 |
| src/proc/proc.c:708-721 | 14 | wakeup唤醒机制 |
| src/proc/swtch.S | 46 | 汇编级上下文切换 |
| src/proc/proc.h:47-98 | 52 | 进程控制块定义 |
| src/syscall/sysproc.c:1273-1302 | 30 | 优先级系统调用 |
| src/trap/trap.c:136-140 | 5 | 时钟中断触发调度 |

### B. 参考文档

- `docs/HWTDocs/调度机制.md` - 详细的调度算法演进文档
- `docs/os-docs/process-and-scheduling.md` - 进程总览文档
- xv6-riscv book - MIT官方教材

### C. 测试用例

- `user/schedtest.c` - 基础调度测试
- `user/priotest.c` - 优先级继承测试
- `user/multicpu.c` - 多核并发测试
- `user/usertests.c` - 综合功能测试

...
1. [项目概述](#项目概述)
2. [VFS虚拟文件系统层设计](#vfs虚拟文件系统层设计)
3. [FAT32文件系统实现](#fat32文件系统实现)
4. [EXT4文件系统实现](#ext4文件系统实现)
5. [文件系统切换机制](#文件系统切换机制)
6. [块设备抽象层](#块设备抽象层)
7. [系统调用集成](#系统调用集成)
8. [性能分析与优化](#性能分析与优化)
...
| 文件路径 | 行数 | 主要功能 |
|---------|------|---------|
| src/fs/fs.h | 100 | VFS数据结构定义 |
| src/fs/fs.c | 750 | xv6fs + VFS分发 |
| src/fs/file.h | 60 | 统一inode结构 |
| src/fs/file.c | 250 | 文件操作接口 |
| src/fs/fat32.h | 50 | FAT32数据结构 |
| src/fs/fat32.c | 1200 | FAT32完整实现 |
| src/fs/ext4fs.h | 40 | EXT4接口定义 |
| src/fs/ext4fs.c | 700 | EXT4适配层 |
| src/fs/bio.c | 180 | 块缓冲区缓存 |
| src/fs/log.c | 250 | 事务日志 |
| src/fs/procfs.c | 120 | ProcFS虚拟文件系统 |

### B. 关键数据结构大小

```
sizeof(struct inode)    = 280字节（含EXT4扩展）
sizeof(struct buf)      = 1080字节（1024B数据+元数据）
sizeof(struct file)     = 32字节
sizeof(struct dirent)   = 16字节（xv6fs）

内存开销（估算）：
  inode缓存：50 × 280B   = 14KB
  块缓存：  30 × 1080B = 32KB
  文件描述符：NFILE × 32B = 3.2KB
  总计：约50KB
```
...
- [lwext4 GitHub](https://github.com/gkostka/lwext4)
- [xv6-riscv book](https://pdos.csail.mit.edu/6.828/)
...
**编写日期**：2026-01-28
**作者**：xv6-lab团队
**关键字**：文件系统、VFS、FAT32、EXT4、虚拟文件系统、块缓存

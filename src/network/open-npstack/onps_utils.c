...
static int onps_atoi(const CHAR *s)
{
	int n = 0;
	int sign = 1;
...
static inline CHAR onps_hex_digit(UCHAR v)
{
	return (v < 10) ? (CHAR)('0' + v) : (CHAR)('A' + (v - 10));
}

static int onps_append_u8_dec(CHAR *dst, int pos, int max, UCHAR val)
{
	if (pos < 0 || pos >= max)
		return -1;
	if (val >= 100)
...
static void onps_ipv4_to_str(const UCHAR *pubAddr, CHAR *pszAddr)
{
	int pos = 0;
	const int max = 20;

	pos = onps_append_u8_dec(pszAddr, pos, max, pubAddr[0]);
	if (pos < 0) goto done;
	pszAddr[pos++] = '.';
	pos = onps_append_u8_dec(pszAddr, pos, max, pubAddr[1]);
	if (pos < 0) goto done;
	pszAddr[pos++] = '.';
	pos = onps_append_u8_dec(pszAddr, pos, max, pubAddr[2]);
	if (pos < 0) goto done;
	pszAddr[pos++] = '.';
	pos = onps_append_u8_dec(pszAddr, pos, max, pubAddr[3]);
...
void snprintf_hex(const UCHAR *pubHexData, USHORT usHexDataLen, CHAR *pszDstBuf, UINT unDstBufSize, BOOL blIsSeparateWithSpace)
{
	...
	for (i = 0; i < usHexDataLen; i++)
	{
		if (i > 0 && blIsSeparateWithSpace)
		{
			if (unFormatBytes + 1 >= unDstBufSize)
				break;
			pszDstBuf[unFormatBytes++] = ' ';
		}

		if (unFormatBytes + 2 >= unDstBufSize)
			break;
		pszDstBuf[unFormatBytes++] = onps_hex_digit((UCHAR)(pubHexData[i] >> 4));
		pszDstBuf[unFormatBytes++] = onps_hex_digit((UCHAR)(pubHexData[i] & 0x0F));
	}

	if (unFormatBytes >= unDstBufSize)
		unFormatBytes = unDstBufSize - 1;
...
{    
	...
	((UCHAR *)&unAddr)[3] = (UCHAR)onps_atoi(pszStart);
    pszStart = pszDot + 1;
    pszDot = mem_char(pszStart, '.', unLen - (pszStart - pszIP));
	((UCHAR *)&unAddr)[2] = (UCHAR)onps_atoi(pszStart);
    pszStart = pszDot + 1;
    pszDot = mem_char(pszStart, '.', unLen - (pszStart - pszIP));
	((UCHAR *)&unAddr)[1] = (UCHAR)onps_atoi(pszStart);
    pszStart = pszDot + 1;
	((UCHAR *)&unAddr)[0] = (UCHAR)onps_atoi(pszStart);
...
char *inet_ntoa(struct in_addr stInAddr)
{
	...
	onps_ipv4_to_str(pubAddr, szAddr);
    return szAddr; 
}
...
{
    UCHAR *pubAddr = (UCHAR *)&stInAddr.s_addr;
	onps_ipv4_to_str(pubAddr, pszAddr);
    return pszAddr;
}

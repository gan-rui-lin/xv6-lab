...
#ifdef ONPS_KERNEL
OS_ADAPTER_EXT void os_critical_enter(void);
OS_ADAPTER_EXT void os_critical_exit(void);
#define os_critical_init()    //* 临界区初始化
#define os_enter_critical()   os_critical_enter()  //* 进入临界区（关中断）
#define os_exit_critical()    os_critical_exit()   //* 退出临界区（开中断）
#else
#define os_critical_init()    //* 临界区初始化
